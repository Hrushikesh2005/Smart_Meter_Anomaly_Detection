...
# Custom JSON provider for MongoDB ObjectId and datetime (Flask 3.0+)
...
def get_weather_analysis():
    ...
    try:
        ...
        for temp_range in temp_analysis.index:
            mean_val = temp_analysis.loc[temp_range, 'mean']
            std_val = temp_analysis.loc[temp_range, 'std']
            count_val = temp_analysis.loc[temp_range, 'count']
...
def predict_anomaly():
    """XGBoost model prediction endpoint"""
    ...
    try:
        ...
        return jsonify({
            'error': 'Model not found. Please train the model first by running: python scripts/train_model.py'
        }), 404
    except Exception as e:
